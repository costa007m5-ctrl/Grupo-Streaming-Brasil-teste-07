...
import { XMarkIcon, ShieldIcon, StarIcon } from './Icons';
        ...
        >
            ...
            <div className={`relative w-full max-w-md rounded-t-2xl p-6 pt-5 shadow-xl transition-transform transform-gpu animate-[slide-up_0.3s_ease-out] ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
                ...
                {(group.host_rating_count ?? 0) > 0 && (
                    <div className={`mt-4 text-center text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                        <div className="flex items-center justify-center space-x-1">
                            <StarIcon className="w-4 h-4 text-yellow-400" solid />
                            <span className="font-bold">{group.host_rating_avg?.toFixed(1)}</span>
                            <span>de avaliação do anfitrião ({group.host_rating_count} opiniões)</span>
                        </div>
                    </div>
                )}
