...
import { PlayIcon, PlusIcon, XMarkIcon, CheckIcon, PauseIcon, BackwardIcon, ForwardIcon, SpeakerWaveIcon, SpeakerXMarkIcon } from '../ui/Icons';
...
interface MaxDetailScreenProps {
    item: ContentItem;
    onBack: () => void;
...
const MaxDetailScreen: React.FC<MaxDetailScreenProps> = ({ item, onBack, onSelectGroup, onSelectItem, isInMyList, addToMyList, removeFromMyList, onViewAllGroups, myGroups, profile, onSendMessage }) => {
    ...
    useEffect(() => {
        ...
        const createPlayer = () => {
            ...
            playerRef.current = new (window as any).YT.Player('youtube-player-max', {
                videoId: trailerKey,
    ...
    const maxUrl = `https://play.max.com/search?q=${encodeURIComponent(title)}`;
    ...
    return (
        <div className="fixed inset-0 bg-black/70 z-30 flex items-center justify-center animate-fade-in">
            ...
            <div className="bg-gradient-to-br from-[#10081C] to-[#1A1D29] w-full max-w-3xl h-[90vh] max-h-[800px] rounded-lg shadow-2xl overflow-y-auto relative scrollbar-hide">
                ...
                <div className="relative h-64 sm:h-80 bg-black">
                    {trailerKey ? (
                        <>
                            <div id="youtube-player-max" className="absolute inset-0 w-full h-full" />
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-[#10081C] via-[#10081C]/70 to-transparent"></div>
                    <div className="absolute bottom-0 left-0 p-6 w-full">
                         ...
                         <div className="flex items-center space-x-3 mt-4">
                            <button onClick={() => setShowJoinOptions(true)} className="flex items-center justify-center space-x-2 bg-white text-black font-bold py-2 px-6 rounded-full hover:bg-gray-200 transition">
                ...
                <div className="p-6 text-white space-y-3">
                    <div className="flex items-center space-x-3 text-sm font-semibold text-gray-300">
                        ...
                        <span>•</span>
                        <span className="text-purple-400 font-bold">{item.vote_average.toFixed(1)} de avaliação</span>
                    </div>
                ...
                {recommendations.length > 0 && (
                    <div className="px-6 py-4 mt-4">
                        <h2 className="text-xl font-bold text-white mb-4">Mais como este</h2>
            ...
            {showJoinOptions && (
                 <div className="fixed inset-0 bg-black/80 z-40 flex items-center justify-center p-4 animate-fade-in">
                    <div className="bg-[#1A1D29] p-6 rounded-lg shadow-xl w-full max-w-sm text-center relative border border-purple-800">
                        ...
                        <div className="space-y-4">
                            <button onClick={() => window.open(maxUrl, '_blank')} className="w-full bg-purple-600 text-white font-bold py-3 rounded-full text-base hover:bg-purple-700 transition">
                                Já tenho conta (Ir para Max)
                            </button>
                            <div className="my-2 text-gray-400 text-sm">OU</div>
                             <div className="space-y-3 text-left">
                                ...
                                <button onClick={onViewAllGroups} className="w-full bg-gray-600 text-white font-bold py-3 rounded-full text-base hover:bg-gray-500 transition">
...
export default MaxDetailScreen;