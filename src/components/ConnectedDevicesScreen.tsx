...
import { CONNECTED_DEVICES_DATA } from '../constants';
...
const ConnectedDevicesScreen: React.FC<{ onBack: () => void; }> = ({ onBack }) => {
    ...
    const handleSignOutFromAllDevices = async () => {
        const confirmation = window.confirm("Você tem certeza que deseja desconectar sua conta de todos os dispositivos, incluindo este?");
        if (confirmation) {
            const { error } = await supabase.auth.signOut({ scope: 'global' });
            if (error) {
                alert("Erro ao desconectar de outros dispositivos: " + error.message);
            } else {
                alert("Desconectado de todos os dispositivos com sucesso.");
                // O app irá deslogar automaticamente devido ao onAuthStateChange
            }
        }
    };
            ...
            <main className="p-4 pt-2 space-y-6">
                <div className="bg-white rounded-xl shadow-sm divide-y divide-gray-100">
                 ...
                 <p className="text-xs text-gray-500 text-center px-4">
                    A lista de dispositivos é ilustrativa. A funcionalidade de desconectar de todos os dispositivos é real.
                </p>
                <div className="text-center">
                     <button onClick={handleSignOutFromAllDevices} className="w-full text-center py-4 bg-red-50 text-red-700 font-semibold rounded-xl hover:bg-red-100 transition-colors text-base">
