...
const AdminScreen: React.FC<AdminScreenProps> = ({ onBack, onInstallApp, showInstallButton, onTestNotification }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const transactionsWithName: Transaction[] = (transactionsRes.data as any[]).map(tx => ({...tx, user_full_name: usersMap.get(tx.user_id) || 'Usuário Deletado' }));
            setAllTransactions(transactionsWithName);

            const ticketsWithUsers: SupportTicket[] = (ticketsRes.data as any[]).map(t => ({...t, user_full_name: usersMap.get(t.user_id) || 'Usuário Deletado', user_avatar_url: usersData.find(u => u.id === t.user_id)?.avatar_url }));
